...
Doelregels van de correctie, elk met bron; de nummers worden in code-commentaar aangehaald. Status per juli 2026 (naronde): **R1–R3 en R12 zitten aantoonbaar in code** — R1 in data + geometry-payload, R2 in de productie-glasmaat (`production.rs`), R3 als tekenoffset in `KozijnCanvas`, R12 in de importfallbacks; **R7–R9** zitten in de profieldata. **Nog open:** R4–R5 (vleugeloverdek en vleugel-glaslijn in het vooraanzicht), R10 (beglazingsblokjes); R6 en R11 zitten gedeeltelijk in de doorsnede-tekenlaag (`profileContour.js`) en het dorpelmodel (`sill.rs`), hun validatieregels nog niet. Van de validatie-uitbreidingen onder de tabel is alleen de glaslathoogte-check (≥ 17) geïmplementeerd.
...
| **R2** | Maatketen glas: sponningmaat = dagmaat + 2×sponninghoogte; **glasmaat = sponningmaat − 2×omtrekspeling** met omtrekspeling **5 mm per zijde** (NPR-minimum; de steldikte zit in de speling, zie C3) — dus glasmaat = dagmaat + 24 bij de 17 mm houtsponning, en de aftrek blijft bij elke sponninghoogte binnen de praktijkcheck (sponningmaat − 6 à 10 mm). NB: de in de onderzoeksronde genoteerde formule "omtrekspeling = max(5; ⅓×sponninghoogte)" is bij de implementatie losgelaten — bij 17 mm geeft die 5,7 mm en conflicteert hij zowel met het gedocumenteerde voorbeeld (+24) als met de praktijkcheck (aftrek > 10 mm bij elke sponning > 15); de ⅓-verhouding staat open als verificatievraag aan de fabriek. Achterhout ≥13 (17-sponning) / ≥15 (20-sponning). | NPR 3577-uitleg; KVT 12.2; glasdiscount-inmeetregel |
...
| **R12** | Import-defaults: de heuristiek `width × 0,36` is verwijderd (schaalde mee met de profielbreedte — root cause van fout #1). Catalogusimport zonder sponningkolom krijgt een vaste waarde per materiaal volgens de veldsemantiek van §7: hout/hout-alu **51** (sponningbreedte vast glas), PVC **28** (Glasfalzhöhe), alu **25** (bibliotheekwaarde, unverified per R1). Onbekend materiaal en DXF-import zonder gedetecteerde sponning schrijven **0 = niet gespecificeerd** (bibliotheekconventie), zodat consumenten per materiaal op de R1-normen terugvallen (`profile::norm_glasinval`) in plaats van op een gegokte maat. | KVT 14.01/DTS; VEKA TPI Softline 82 MD; eigen constatering (`dxf_profile.rs`, `catalog.rs`) |
